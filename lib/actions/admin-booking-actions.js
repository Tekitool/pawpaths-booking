...
async function mapSupabaseToPage(booking) {
    ...
    // Sign URLs for extra pets
    for (const pId of Object.keys(extraDocuments)) {
        if (extraDocuments[pId].passport) extraDocuments[pId].passportUrl = await getSignedUrl(extraDocuments[pId].passport);
        if (extraDocuments[pId].vaccination) extraDocuments[pId].vaccinationUrl = await getSignedUrl(extraDocuments[pId].vaccination);
        if (extraDocuments[pId].rabies) extraDocuments[pId].rabiesUrl = await getSignedUrl(extraDocuments[pId].rabies);
    }

    // Map Pets
    const pets = booking.booking_pets?.map(bp => {
        // Resolve photo: photos JSONB array (primary) → photo_path string (legacy) → booking fallback
        const photosArr = bp.pet.photos
        let photoUrl = null
        ...
        if (!photoUrl && bp.pet.photo_path) {
            photoUrl = getPublicUrl(STORAGE_BUCKETS.PHOTOS, bp.pet.photo_path)
        }
        if (!photoUrl) {
        ...
        return {
            ...
            medicalAlerts: bp.pet.medical_alerts || [],         // JSONB array from pets table
    ...
    const origin = typeof booking.origin_raw === 'string' ? JSON.parse(booking.origin_raw) : booking.origin_raw || {};
    const destination = typeof booking.destination_raw === 'string' ? JSON.parse(booking.destination_raw) : booking.destination_raw || {};
