...
export async function getUsers() {
    try {
        ...
        return users.map(user => ({
            ...
            avatar: user.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(user.name)}&background=random` // Use stored avatar or fallback
        }));
    } catch (error) {
...
export async function createUser(userData) {
    try {
        ...
        const { name, email, password, role, status, avatar } = sanitize(userData);
...
export async function updateUser(userId, userData) {
    try {
        ...
        const { name, email, role, status, password, avatar } = sanitize(userData);
