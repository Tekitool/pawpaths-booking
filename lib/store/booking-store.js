...
const useBookingStore = create(
    persist(
        (set) => ({
            ...
            resetForm: () => set({
                currentStep: 1,
                _cache: { species: null, breeds: null, genderOptions: null, services: null, servicesKey: null },
                formData: {
                    travelDetails: {
                        originCountry: '',
                        originAirport: '',
            ...
            // ── Prefetch Cache (ephemeral — NOT persisted to localStorage) ─────
            // Populated by usePrefetchWizardData hook in the background.
            // Step components read cache-first, fallback to live fetch.
            _cache: {
                species: null,       // Array<{ id, name, iata_code }>
                breeds: null,        // Array<{ id, species_id, name, default_image_path }>
                genderOptions: null, // Array<{ value, label }>
                services: null,      // Array<Service> from get_valid_services RPC
                servicesKey: null,    // cache-invalidation key e.g. "export|manifest_cargo|Dog,Cat"
            },
            setCacheData: (key, data) =>
                set((state) => ({ _cache: { ...state._cache, [key]: data } })),
            clearCache: () =>
                set({
                    _cache: {
                        species: null, breeds: null, genderOptions: null,
                        services: null, servicesKey: null,
                    },
                }),
        ...
        }),
        {
            ...
            partialize: (state) => {
                ...
                // Exclude _cache (ephemeral) and petFiles (non-serialisable) from storage
                const { _cache, ...rest } = state;
                return { ...rest, formData: restFormData };
            },
            onRehydrateStorage: () => (state, error) => {
                if (error) {
                    console.error('Booking store rehydration failed, clearing:', error);
                    try { localStorage.removeItem('pawpaths-booking-storage'); } catch (_) {}
                    return;
                }
                // Validate rehydrated state has expected shape
                if (state && (!state.formData || !state.formData.travelDetails)) {
                    console.warn('Booking store has invalid shape, resetting');
                    state.resetForm?.();
                }
            },
        }
